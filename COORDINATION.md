...
6. **T-028 Production-log dedupe on PRODUCTION** — UPDATED 2026-05-12: Work on production sheet (1LP7...), not staging. First copy entire Production Log to `_archive_ProductionLog_2026-05-12`, then dedupe in place by (Submitted At + Date + Dish + Batch ID) composite key. Document: row count before/after, number of duplicates removed.
7. **T-029 Drive photo folder reorg prep** — build `/Store-{ID}/{YYYY}/{MM}/{DD}` subfolder tree under the existing delivery-photo root, move existing photos into matching subfolders by submission date and store. **Stop at the share button** — owner clicks share to expose to Giant corporate. Document: number of photos moved.
